                               ...
                               float alpha)
{
	...
	Renderer::SceneUniformData sceneParams{
	    proj, view, glm::vec4{ambient, 0.0f}, glm::vec4{dynamic, 0.0f},
	    glm::vec4(skyBottom, 1.f), glm::vec4(camera.position, 0.f),
	    weather.fogStart, _camera.frustum.far};
...
void GameRenderer::renderEffects(GameWorld* world)
{
	...
	for (VisualFX* fx : effects) {
		...
		if (fx->getType() != VisualFX::Particle)
			continue;
