...
void ScriptMachine::interuptNext() { interupt = true; }
...
void ScriptMachine::executeThread(SCMThread& t, int msPassed)
{
	...
	while (t.wakeCounter == 0) {
		...
		if (strcmp(t.name, "EIGHT") == 0) {
			printf("% 8s  %04x %04x % 25s", t.name, t.programCounter, opcode,
...
SCMBreakpointInfo* ScriptMachine::findBreakpoint(SCMThread& t,
                                                 SCMThread::pc_t pc)
{
	for (std::vector<SCMBreakpointInfo>::iterator bp = breakpoints.begin();
	     bp != breakpoints.end(); ++bp) {
		if ((bp->breakpointFlags & SCMBreakpointInfo::BP_ProgramCounter) ==
		    SCMBreakpointInfo::BP_ProgramCounter) {
			if (bp->programCounter != pc) {
				continue;
			}
		}
		if ((bp->breakpointFlags & SCMBreakpointInfo::BP_ThreadName) ==
		    SCMBreakpointInfo::BP_ThreadName) {
...
void ScriptMachine::setBreakpointHandler(
    const ScriptMachine::BreakpointHandler& handler)
{
	bpHandler = handler;
}
