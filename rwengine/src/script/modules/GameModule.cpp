...
void game_create_vehicle_generator(const ScriptArguments& args)
{
	...
	if (args[4].type == TString) {
		args.getWorld()->logger->error(
		    "SCM", "Model names not supported for vehicle generator");
		return;
	}
...
{
	RW_CHECK(args.getWorld()->state->vehicleGenerators.size() >
	             *args[0].globalInteger,
	         "VehicleGenerator out of range");
	if (args.getWorld()->state->vehicleGenerators.size() >
	    *args[0].globalInteger) {
		VehicleGenerator& generator =
		    args.getWorld()->state->vehicleGenerators.at(
		        *args[0].globalInteger);
...
void game_set_zone_car_info(const ScriptArguments& args)
{
	...
	if (it != args.getWorld()->data->zones.end()) {
		...
		for (size_t i = 2;
		     i < args.getParameters().size() && i - 2 < ZONE_GANG_COUNT; ++i) {
			if (day) {
...
{
	auto character =
	    static_cast<CharacterObject*>(args.getObject<CharacterObject>(0));
...
void game_camera_lookat_position(const ScriptArguments& args)
{
	...
	auto direction =
	    glm::normalize(position - args.getWorld()->state->cameraPosition);
	auto right =
...
{
	args.getWorld()->state->scriptOnMissionFlag =
	    (unsigned int*)args[0].globalInteger;
}
...
void game_enable_input(const ScriptArguments& args)
{
	...
	static_cast<PlayerController*>(player->controller)
	    ->setInputEnabled(!!args[1].integer);
}
...
{
	std::string id(args[0].string);
	std::string str = ScreenText::format(args.getWorld()->data->texts.text(id),
	                                     formatValue(args[1]));
...
bool game_is_car_inside_garage(const ScriptArguments& args)
{
	...
	RW_CHECK(garageIndex < static_cast<int>(garages.size()),
	         "Garage index too high");
	const auto& garage = garages[garageIndex];

	for (auto& v : gw->vehiclePool.objects) {
		// @todo if this car only accepts mission cars we probably have to
		// filter here / only check for one specific car
		auto vp = v.second->getPosition();
		if (vp.x >= garage.min.x && vp.y >= garage.min.y &&
		    vp.z >= garage.min.z && vp.x <= garage.max.x &&
		    vp.y <= garage.max.y && vp.z <= garage.max.z) {
			return true;
		}
	}
...
bool game_garage_contains_car(const ScriptArguments& args)
{
	...
	auto vehicle =
	    static_cast<VehicleObject*>(args.getObject<VehicleObject>(1));
	if (vehicle) {
		/// @todo if this car only accepts mission cars we probably have to
		/// filter here / only check for one specific car
...
{
	auto chartype =
	    args.getWorld()->data->findObjectType<CharacterData>(args[0].integer);
	if (chartype) {
		auto modelfind =
		    args.getWorld()->data->models.find(chartype->modelName);
...
void game_restart_critical_mission(const ScriptArguments& args)
{
	...
	auto object =
	    args.getWorld()->pedestrianPool.find(args.getState()->playerObject);
...
/// @todo http://www.gtamodding.com/index.php?title=0256 (e.g. check if dead or
/// busted)
bool game_is_player_playing(const ScriptArguments& args)
{
	auto character = args.getPlayerCharacter(0);
	return character != nullptr;
	// return args.getWorld()->findObject(args.getState()->playerObject) !=
	// nullptr;
}
...
static const char* sprite_names[] = {
    "",  // 0
    "radar_asuka", "radar_bomb", "radar_cat", "radar_centre", "radar_copcar",
    "radar_don", "radar_eight", "radar_el", "radar_ice", "radar_joey",
    "radar_kenji", "radar_liz", "radar_luigi", "radar_north", "radar_ray",
    "radar_sal", "radar_save", "radar_spray", "radar_tony", "radar_weapon",
};
...
void game_create_cutscene_object(const ScriptArguments& args)
{
	...
	if (object == nullptr) {
		args.getWorld()->logger->error(
		    "SCM", "Could not create cutscene object " + std::to_string(id));
	} else {
...
{
	float time = args.getWorld()->getGameTime() -
	             args.getWorld()->state->cutsceneStartTime;
	if (args.getWorld()->state->skipCutscene) {
		*args[0].globalInteger =
		    args.getWorld()->state->currentCutscene->tracks.duration * 1000;
	} else {
...
void game_create_crusher_crane(const ScriptArguments& args)
{
	...
	/// @todo check how to store all parameters and how to create the actual
	/// crusher
...
void game_set_character_model(const ScriptArguments& args)
{
	...
	if (character) {
		static_cast<CharacterObject*>(character)
		    ->changeCharacterModel(args[1].string);
	}
}
...
#define CHASE_VEHICLE(model, x, y, z, hdg, c1, c2, path)                  \
	{                                                                     \
		...
		args.getWorld()->chase.addChaseVehicle(                           \
		    vehicle0, path, args.getWorld()->data->getDataPath() +        \
...
bool game_collision_loaded(const ScriptArguments& args)
{
	...
	/// @todo use the current player zone island number to return the correct
	/// value.
	RW_UNUSED(args);
...
void game_play_music_id(const ScriptArguments& args)
{
	...
	RW_UNUSED(id);
	RW_UNIMPLEMENTED(
	    "game_play_music_id(): should be play mission passed tune");
	GameWorld* gw = args.getWorld();
...
void game_clear_area(const ScriptArguments& args)
{
	...
	if (clearParticles) {
		...
		RW_UNIMPLEMENTED(
		    "game_clear_area(): should clear all particles and projectiles "
		    "(not limited to area!)");
	}
}
...
{
	*args[0].globalInteger =
	    args.getWorld()->state->playerInfo.hiddenPackagesCollected;
}
...
void game_display_help(const ScriptArguments& args)
{
	...
	args.getWorld()->state->text.addText<ScreenTextType::Big>(
	    ScreenTextEntry::makeHelp(id, str));
}
...
{
	// Collision is loaded when required, not sure what this is supposed to
	// mean.
	RW_UNUSED(args);
...
GameModule::GameModule() : ScriptModule("Game")
{
	...
	bindUnimplemented(0x0109, game_add_character_money, 2,
	                  "Add Character Money");
	...
	bindFunction(0x0112, game_has_death_or_arrest_finished, 0,
	             "Is Death or Arrest Finished");

	bindFunction(0x014B, game_create_vehicle_generator, 13,
	             "Create Car Generator");
	bindFunction(0x014C, game_set_vehicle_generator_count, 2,
	             "Set Car Generator count");
	bindUnimplemented(0x014D, add_pager_message, 4, "show pager message");
	bindUnimplemented(0x014E, game_show_onscreen_timer, 1,
	...
	bindUnimplemented(0x0158, game_camera_follow_vehicle, 3,
	                  "Camera Follow Vehicle");
	bindFunction(0x0159, game_camera_follow_character, 3,
	...
	bindFunction(0x015F, game_camera_fixed_position, 6,
	             "Set Fixed Camera Position");
	bindFunction(0x0160, game_camera_lookat_position, 4,
	...
	bindUnimplemented(0x0181, game_link_character_mission_flag, 2,
	                  "Link Character Mission Flag");
	bindUnimplemented(0x0182, game_unknown, 2, "Unknown Character Opcode");

	bindFunction(0x0186, game_add_object_blip<VehicleObject>, 2,
	             "Add Blip for Vehicle");
	bindFunction(0x0187, game_add_object_blip<CharacterObject>, 2,
	             "Add Blip for Character");
	...
	bindFunction(0x01E3, game_print_big_with_number, 4,
	             "Print Big With Number");

	bindFunction(0x01E7, game_disable_roads, 6, "Enable Roads");
	bindFunction(0x01E8, game_enabled_roads, 6, "Disable Roads");

	bindUnimplemented(0x01EB, game_set_traffic_multiplier, 1,
	                  "Set Traffic Density Multiplier");

	bindUnimplemented(0x01ED, game_clear_threat_search, 1,
	                  "Clear Character Threat Search");
	...
	bindUnimplemented(0x01F7, game_police_ignore_character, 2,
	                  "Set Cops Ignore Player");
	...
	bindUnimplemented(0x022D, game_character_face_player, 2,
	                  "Set Character Always Face Player");

	bindUnimplemented(0x022F, game_stop_character_look, 1,
	                  "Stop Character Looking");
	...
	bindFunction(0x023D, game_special_char_loaded, 1,
	             "Is Special Character Loaded");

	bindFunction(0x0244, game_cutscene_offset, 3, "Set Cutscene Offset");
	bindUnimplemented(0x0245, game_set_character_anims, 2,
	                  "Set Character Animation Group");
	...
	bindFunction(0x0255, game_restart_critical_mission, 4,
	             "Restart Critical Mission");
	...
	bindUnimplemented(0x0296, game_unload_special_characters, 1,
	                  "Unload Special Character");
	...
	bindFunction(0x02E5, game_create_cutscene_object, 2,
	...
	bindUnimplemented(0x02EC, game_create_hidden_package, 3,
	                  "Create Hidden Package");

	bindFunction(0x02ED, game_set_hidden_packages, 1,
	             "Set Total Hidden Packages");

	bindFunction(0x02F3, game_load_special_model, 2, "Load Special Model");
	bindFunction(0x02F4, game_create_cutscene_head, 3,
	             "Create Cutscene Actor Head");
	bindFunction(0x02F5, game_set_head_animation, 2,
	             "Set Cutscene Head Animation");
	...
	bindUnimplemented(0x0317, game_increment_mission_attempts, 0,
	                  "Increment Mission Attempts");
	bindFunction(0x0318, game_set_last_mission, 1,
	...
	bindUnimplemented(0x032B, game_create_weapon_pickup, 7,
	                  "Create Weapon Pickup");

	bindUnimplemented(0x0335, game_free_resprays, 1, "Set Free Respray");
	bindUnimplemented(0x0336, game_set_character_visible, 2,
	                  "Set Player Visible");
	...
	bindUnimplemented(0x0344, game_set_center_text_size, 1,
	                  "Set Center Text Size");
	bindUnimplemented(0x0345, game_set_text_background, 1,
	                  "Set Text Background");
	bindFunction(0x0346, game_set_background_colour, 4,
	             "Set Text Background Colour");

	bindUnimplemented(0x0348, game_set_text_size_proportional, 1,
	                  "Set Text Size Proportional");
	bindUnimplemented(0x0349, game_set_font, 1, "Set Text Font");

	bindFunction(0x0352, game_set_character_model, 2, "Set Character Model");
	bindUnimplemented(0x0353, game_refresh_character_model, 1,
	                  "Refresh Actor Model");
	...
	bindUnimplemented(0x0373, game_camera_behind_player, 0,
	                  "Set Camera Behind Player");
	bindUnimplemented(0x0374, game_set_motion_blur, 1, "Set Motion Blur");

	bindUnimplemented(0x038B, game_load_models_now, 0,
	                  "Load Requested Models Now");

	bindFunction(0x0394, game_play_music_id, 1, "Play music");
	bindFunction(0x0395, game_clear_area, 5,
	             "Clear Area Vehicles and Pedestrians");

	bindUnimplemented(0x0397, game_set_vehicle_siren, 2, "Set Vehicle Siren");

	bindUnimplemented(0x0399, game_disable_ped_paths_in_area, 7,
	                  "Disable ped paths in angled cube");

	bindUnimplemented(0x039D, game_scatter_particles, 12, "Scatter Particles");
	bindUnimplemented(0x039E, game_set_character_hijackable, 2,
	                  "Set Character can be dragged out");

	bindUnimplemented(0x03AD, game_set_rubbish_enabled, 1,
	                  "Set Rubbish Enabled");
	bindUnimplemented(0x03AE, game_clear_area_particles, 6,
	                  "Remove Particles in Area");
	bindUnimplemented(0x03AF, game_set_streaming_enabled, 1,
	                  "Set Map Streaming Enabled");

	bindUnimplemented(0x03B7, game_custscen_only, 1, "Process Cutscene Only");

	bindUnimplemented(0x03BB, game_set_garage_rotating, 1,
	                  "Set Garage Door to Rotate");

	bindUnimplemented(0x03BF, game_set_peds_ignore_player, 2,
	                  "Set Pedestrians Ignoring Player");

	bindUnimplemented(0x03C1, game_store_character_vehicle_nosave, 2,
	                  "Store Player Vehicle No-Save");

	bindUnimplemented(0x03C4, game_display_counter_message, 3,
	                  "Display Counter Message");
	bindUnimplemented(0x03C5, game_create_vehicle_parked, 4,
	                  "Spawn Parked Vehicle");
	bindFunction(0x03C6, game_collision_loaded, 1, "Is Collision In Memory");

	bindUnimplemented(0x03C8, game_camera_infront_player, 0,
	                  "Put Camera Infront of player");
	...
	bindFunction(0x03D2, game_is_audio_finished, 0,
	             "Is Mission Audio Finished");
	...
	bindUnimplemented(0x03DA, game_set_garage_follow_player, 1,
	                  "Set Garage Camera Follows Player");

	bindFunction(0x03DC, game_add_object_blip<PickupObject>, 2,
	             "Add blip for pickup");

	bindUnimplemented(0x03DE, game_set_pedestrian_density, 1,
	                  "Set Pedestrian density");

	bindFunction(0x03E1, game_get_found_hidden_packages, 1,
	             "Get Hidden Packages Found");
	...
	bindFunction(0x03EC, game_has_crane_collected_all_cars, 0,
	             "Has Crane Collected All Cars");

	bindFunction(0x03EE, game_can_player_move, 1, "Can Player Move");
	bindUnimplemented(0x03EF, game_safe_player, 1,
	                  "Make Player Safe For Cutscene");

	bindUnimplemented(0x03F0, game_enabled_text, 1, "Enable Text Draw");
	bindUnimplemented(0x03F1, game_set_character_hostility, 2,
	                  "Set Ped Hostility");
	bindUnimplemented(0x03F2, game_clear_character_hostility, 2,
	                  "Clear Ped Hostility");
	...
	bindUnimplemented(0x0418, game_set_object_ontop, 2,
	                  "Set Object Draw Ontop");
	...
	bindFunction(0x0445, game_are_vehicle_cheats_enabled, 0,
	             "Are Any Vehicle Cheats enabled");

	bindFunction(0x044D, game_load_splash, 1, "Load Splash Screen");

	bindUnimplemented(0x0452, game_enable_user_camera, 0,
	                  "Enable User Camera Controll");
}
