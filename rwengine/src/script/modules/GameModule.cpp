...
void game_create_vehicle_generator(const ScriptArguments& args)
{
  ...
  if (args[4].type == TString) {
    args.getWorld()->logger->error("SCM",
                                   "Model names not supported for vehicle generator");
    return;
  }
  ...
  VehicleGenerator vg{position, heading, vehicleID, colourFG, colourBG, alwaysSpawn,
                      alarmThreshold, lockedThreshold, minDelay, maxDelay, 0, 0};
...
void game_print_big_with_number(const ScriptArguments& args)
{
  std::string id(args[0].string);
  std::string str =
      ScreenText::format(args.getWorld()->data->texts.text(id), formatValue(args[1]));
...
bool game_is_car_inside_garage(const ScriptArguments& args)
{
  ...
  for (auto& v : gw->vehiclePool.objects) {
    // @todo if this car only accepts mission cars we probably have to filter here / only
    // check for one specific car
...
static const char* sprite_names[] = {
    ...
    "radar_eight", "radar_el", "radar_ice", "radar_joey", "radar_kenji", "radar_liz",
    "radar_luigi", "radar_north", "radar_ray", "radar_sal", "radar_save", "radar_spray",
    "radar_tony", "radar_weapon",
};
...
bool game_cutscene_finished(const ScriptArguments& args)
{
  if (args.getWorld()->state->currentCutscene) {
    float time =
        args.getWorld()->getGameTime() - args.getWorld()->state->cutsceneStartTime;
    if (args.getWorld()->state->skipCutscene) {
      return true;
    }
...
void game_print_big_with_2_numbers(const ScriptArguments& args)
{
  ...
  std::string str = ScreenText::format(world->data->texts.text(id), formatValue(args[1]),
                                       formatValue(args[2]));
...
void game_clear_area(const ScriptArguments& args)
{
  ...
  /// @todo Do we also have to clear all projectiles + particles *in this area*, even if
  /// the bool is false?

  if (clearParticles) {
    RW_UNUSED(clearParticles);
    RW_UNIMPLEMENTED(
        "game_clear_area(): should clear all particles and projectiles (not limited to "
        "area!)");
  }
}
...
GameModule::GameModule() : ScriptModule("Game")
{
  ...
  bindFunction(0x03DD, game_add_object_sprite_blip<PickupObject>, 3,
               "Add Sprite Blip for Pickup");
