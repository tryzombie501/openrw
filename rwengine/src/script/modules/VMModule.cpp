...
{
	// Negative jump addresses indicate a jump relative to the start of the
	// thread.
...
void vm_jump(const ScriptArguments& args)
{
	args.getThread()->programCounter =
	    localizeLabel(args.getThread(), args[0].integer);
}
...
{
	args.getThread()->conditionResult =
	    args[0].integer > *args[1].globalInteger;
}
...
VMModule::VMModule() : ScriptModule("VM")
{
	...
	bindFunction(0x020, vm_global_float_gt_float, 2,
	             "Global Float Greather than Float");
	...
	bindFunction(0x042, vm_global_float_eq_float, 2,
	             "Global Float Equal to Float");
	...
	bindFunction(0x086, vm_global_float_to_global, 2,
