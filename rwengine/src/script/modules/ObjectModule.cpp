...
bool game_player_in_area_2d(const ScriptArguments& args)
{
	...
	if (player.x > min.x && player.y > min.y && player.x < max.x &&
	    player.y < max.y) {
		return true;
	}
	return false;
...
void game_create_character(const ScriptArguments& args)
{
	...
	auto character =
	    args.getWorld()->createPedestrian(id, position + spawnMagic);
...
{
	auto vdata =
	    args.getWorld()->data->findObjectType<VehicleData>(args[1].integer);
	if (vdata) {
		auto character =
...
bool game_player_in_model(const ScriptArguments& args)
{
	...
	if (vdata) {
		auto character =
		    static_cast<CharacterObject*>(args.getPlayerCharacter(0));
		auto vehicle = character->getCurrentVehicle();
		if (vehicle) {
			return vehicle->model && vdata->modelName == vehicle->model->name;
		}
...
bool game_player_in_area_2d_in_vehicle(const ScriptArguments& args)
{
	...
	if (drawCylinder) {
		auto ground =
		    args.getWorld()->getGroundAtPosition(glm::vec3(position, 100.f));
		args.getWorld()->drawAreaIndicator(AreaIndicatorInfo::Cylinder,
		                                   ground + glm::vec3(0.f, 0.f, 4.5f),
		                                   glm::vec3(radius, 5.f));
	}
...
bool game_character_near_point_on_foot_3D(const ScriptArguments& args)
{
	...
	if (drawCylinder) {
		args.getWorld()->drawAreaIndicator(AreaIndicatorInfo::Cylinder, center,
		                                   size);
	}
...
{
	auto vehicle =
	    static_cast<VehicleObject*>(args.getObject<VehicleObject>(0));
	auto type = args[1].integer;
	RW_UNUSED(type);
	RW_UNIMPLEMENTED("game_create_character_in_vehicle(): character type");
	auto id = args[2].integer;

	auto character = args.getWorld()->createPedestrian(
	    id, vehicle->getPosition() + spawnMagic);
	new DefaultAIController(character);
...
bool game_character_in_area_or_cylinder(const ScriptArguments& args)
{
	...
	RW_UNIMPLEMENTED(
	    "game_character_in_area_or_cylinder: should use cylinder if "
	    "requested?");
...
bool game_character_stoped_in_volume_in_vehicle(const ScriptArguments& args)
{
	...
	if (character && character->getCurrentVehicle() != nullptr) {
		...
		if (pp.x >= min.x && pp.y >= min.y && pp.z >= min.z && pp.x <= max.x &&
		    pp.y <= max.y && pp.z <= max.z) {
			return character->getCurrentVehicle()
			           ->physVehicle->getCurrentSpeedKmHour() < 0.75f;
		}
...
bool game_character_stoped_in_volume(const ScriptArguments& args)
{
	...
	if (drawCylinder) {
		args.getWorld()->drawAreaIndicator(
		    AreaIndicatorInfo::Cylinder, (max + min) / 2.f, (max - min) / 2.f);
	}
...
bool game_player_in_taxi(const ScriptArguments& args)
{
	...
	auto vehicle = character->getCurrentVehicle();
	return (vehicle &&
	        (vehicle->vehicle->classType & VehicleData::TAXI) ==
	            VehicleData::TAXI);
}
...
void game_enter_as_driver(const ScriptArguments& args)
{
	...
	character->controller->setNextActivity(
	    new Activities::EnterVehicle(vehicle, 0));
}
...
void game_enter_as_passenger(const ScriptArguments& args)
{
	...
	character->controller->setNextActivity(new Activities::EnterVehicle(
...
void game_character_follow_character(const ScriptArguments& args)
{
	auto character =
	    static_cast<CharacterObject*>(args.getObject<CharacterObject>(0));
	auto leader =
	    static_cast<CharacterObject*>(args.getObject<CharacterObject>(1));
...
{
	PickupObject* pickup =
	    static_cast<PickupObject*>(args.getObject<PickupObject>(0));
	...
	if (id < 0) {
		...
		if (id == (uint16_t)-1) {
			args.getWorld()->logger->error("SCM",
			                               "Failed to find model " + modelname);
		}
	}
...
ObjectModule::ObjectModule() : ScriptModule("Object")
{
	...
	bindFunction(0x0055, game_set_object_position<PlayerController>, 4,
	             "Set Player Position");
	...
	bindFunction(0x009B, game_destroy_object<CharacterObject>, 1,
	             "Destroy Character");
	bindUnimplemented(0x009C, game_set_character_wander_path, 2,
	                  "Set Character Wander Path");

	bindUnimplemented(0x009F, game_character_make_idle, 1,
	                  "Set Character to Idle");

	bindFunction(0x00A1, game_set_object_position<CharacterObject>, 4,
	             "Set Character Position");

	bindFunction(0x00A5, game_create_vehicle, 5, "Create Vehicle");
	bindFunction(0x00A6, game_destroy_object<VehicleObject>, 1,
	             "Destroy Vehicle");

	bindFunction(0x00AA, game_get_vehicle_position, 4, "Get Vehicle Position");

	bindFunction(0x00D9, game_get_character_vehicle<CharacterObject>, 2,
	             "Get Character Vehicle");
	bindFunction(0x00DA, game_get_character_vehicle<PlayerController>, 2,
	             "Get Player Vehicle");
	bindFunction(0x00DB, game_character_in_vehicle<CharacterObject>, 2,
	             "Is Character in Vehicle");
	bindFunction(0x00DC, game_character_in_vehicle<PlayerController>, 2,
	             "Is Player in Vehicle");

	bindFunction(0x00DE, game_player_in_model, 2, "Is Player In Model");
	bindFunction(0x00DF, game_character_in_any_vehicle, 1,
	             "Is Character In Any Vehicle");
	bindFunction(0x00E0, game_player_in_any_vehicle, 1,
	             "Is Player In Any Vehicle");

	bindFunction(0x00E5, game_player_in_area_2d_in_vehicle, 6,
	             "Is Player in 2D Area in Vehicle");

	bindUnimplemented(0x00E4, game_locate_character_on_foot_2d, 6,
	                  "Locate Player on foot 2D");
	...
	bindFunction(0x00E9, game_character_near_character_2D, 5,
	             "Locate Character near Character 2D");

	bindFunction(0x00EB,
	             game_character_near_character_in_vehicle_2D<PlayerController>,
	             5, "Is player near character in vehicle");

	bindFunction(0x00ED, game_character_near_point_on_foot_2D, 6,
	             "Is Character near point on foot");

	bindFunction(0x00EF, game_locate_character_stopped_2d<CharacterObject>, 6,
	             "Locate Character Stopped 2D");

	bindUnimplemented(0x00F5, game_locate_character_in_sphere, 8,
	                  "Locate Player In Sphere");
	bindFunction(0x00F6, game_player_near_point_on_foot_3D, 8,
	             "Is Player near point on foot");
	bindFunction(0x00F7, game_player_near_point_in_vehicle_3D, 8,
	             "Is Player near point in car");
	bindFunction(0x00F9, game_player_stopped_near_point_on_foot_3d, 8,
	             "Detect player stopped on foot area 3d");

	bindFunction(0x0100, game_character_near_point_in_vehicle, 8,
	             "Is Character near point in car");

	bindFunction(0x0108, game_destroy_object<InstanceObject>, 1,
	             "Destroy Object");

	bindFunction(0x0118, game_character_dead, 1, "Is Character Dead");
	bindFunction(0x0119, game_vehicle_dead, 1, "Is Vehicle Dead");
	bindUnimplemented(0x011A, game_set_character_search_threat, 2,
	                  "Set Character Search Threat");

	bindUnimplemented(0x011C, game_character_clear_objective, 1,
	                  "Clear Character Objective");

	bindFunction(0x0121, game_character_in_zone<PlayerController>, 2,
	             "Is Player In Zone");
	bindFunction(0x0122, game_player_pressing_horn, 1,
	             "Is Player Pressing Horn");

	bindFunction(0x0126, game_character_objective_passed, 1,
	             "Character Objective Passed");

	bindFunction(0x0129, game_create_character_in_vehicle, 4,
	             "Create Character In Car");

	bindUnimplemented(0x0135, game_set_vehicle_locked, 2,
	                  "Set Vehicle locked state");
	...
	bindFunction(0x0173, game_set_object_heading<CharacterObject>, 2,
	             "Set Character Heading");
	bindUnimplemented(0x0174, game_get_character_heading, 2,
	                  "Get Vehicle Heading");
	bindFunction(0x0175, game_set_object_heading<VehicleObject>, 2,
	             "Set Vehicle heading");

	bindFunction(0x0176, game_get_object_heading<InstanceObject>, 2,
	             "Get Object heading");
	bindFunction(0x0177, game_set_object_heading<InstanceObject>, 2,
	             "Set Object heading");

	bindUnimplemented(0x0192, game_character_stand_still, 1,
	                  "Make character stand still");

	bindFunction(0x0199, game_character_in_area_or_cylinder<PlayerController>,
	             6, "Is Player in Area or Cylinder");
	...
	bindFunction(0x01A0,
	             game_character_stoped_in_volume_in_vehicle<PlayerController>,
	             8, "Is Player Stopped in cube in vehicle");
	bindFunction(0x01A8, game_character_stoped_in_volume<CharacterObject>, 8,
	             "Is Char Stopped in volume");
	bindFunction(0x01AA,
	             game_character_stoped_in_volume_in_vehicle<CharacterObject>, 8,
	             "Is Char Stopped in cube in vehicle");

	bindFunction(0x01B2, game_give_weapon_to_character, 3,
	             "Give Weapon to Character");

	bindUnimplemented(0x01BB, game_object_coordinates, 4,
	                  "Get Object Coordinates");

	bindUnimplemented(0x01BE, game_turn_character, 4,
	                  "Turn Character To Face Point");
	...
	bindFunction(0x01D5, game_enter_as_driver, 2,
	             "Character Enter Vehicle as Driver");
	bindFunction(0x01D4, game_enter_as_passenger, 2,
	             "Character Enter Vehicle as Passenger");
	bindFunction(0x01D3, game_character_exit_vehicle, 2,
	             "Character Exit Vehicle");

	bindFunction(0x01DF, game_character_follow_character, 2,
	             "Make Character Follow Character");

	bindFunction(0x01F3, game_vehicle_in_air, 1, "Is Vehicle In Air");
	bindFunction(0x01F4, game_vehicle_flipped, 1, "Is Car Flipped");

	bindFunction(0x0204, game_character_near_car_2d, 5,
	             "Is Char near Car in Car 2D");
	...
	bindFunction(0x0214, game_is_pickup_collected, 1,
	             "Has Pickup been collected");
	...
	bindFunction(0x029B, game_create_object_world, 5,
	             "Create Object no offset");
	bindFunction(0x029C, game_is_boat, 1, "Is Vehicle Boat");

	bindFunction(0x029F, game_is_character_stopped<PlayerController>, 1,
	             "Is Player Stopped");

	bindFunction(0x02B3, game_character_in_area_9<PlayerController>, 9,
	             "Is Player In Area");
	...
	bindFunction(0x0320, game_character_in_range, 2,
	             "Is Character in range of character");
	...
	bindUnimplemented(0x035D, game_set_object_targetable, 1,
	                  "Set Object Targetable");

	bindFunction(0x0363, game_set_close_object_visible, 6,
	             "Set Closest Object Visibility");

	bindUnimplemented(0x0368, game_create_ev_crane, 10, "Create ev Crane");

	bindFunction(0x03B6, game_change_nearest_model, 6,
	             "Change Nearest Instance Model");

	bindUnimplemented(0x03BA, game_clear_area_vehicles, 6,
	                  "Clear Cars From Area");

	bindUnimplemented(0x03BC, game_create_cylinder, 5,
	                  "Create Cylinder");  /// @todo Other sources call this
	                                       /// "sphere", this should be our
	                                       /// AreaIndicator?
	...
	bindUnimplemented(0x03F9, game_make_characters_converse, 3,
	                  "Make characters converse");

	bindUnimplemented(0x042B, game_clear_volume_pedestrians, 6,
	                  "Clear volume pedestrians");

	bindFunction(0x0442, game_character_in_vehicle<PlayerController>, 2,
	             "Is Player in This Vehicle");
