...
void game_create_character_in_vehicle(const ScriptArguments& args)
{
  ...
  auto character =
      args.getWorld()->createPedestrian(id, vehicle->getPosition() + spawnMagic);
  new DefaultAIController(character);
...
bool game_character_in_area_or_cylinder(const ScriptArguments& args)
{
  ...
  RW_UNIMPLEMENTED(
      "game_character_in_area_or_cylinder: should use cylinder if requested?");
...
bool game_player_in_taxi(const ScriptArguments& args)
{
  ...
  auto vehicle = character->getCurrentVehicle();
  return (vehicle &&
          (vehicle->vehicle->classType & VehicleData::TAXI) == VehicleData::TAXI);
}
...
void game_get_random_character_in_zone(const ScriptArguments& args)
{
  ...
  if (zfind != zones.end()) {
    // Create a list of candidate characters by iterating and checking if the char is in
    // this zone
    ...
    if (candidateCount > 0) {
      // Return the handle for any random character in this zone and use lifetime for use
      // by script
...
void game_create_character_as_passenger(const ScriptArguments& args)
{
  ...
  if (seat <= -1) {
    ...
    /// @todo 0 - passenger seat (or back seat of bike); 1 - left rear seat; 2 - right
    /// rear seat
    seat++;
  }
...
ObjectModule::ObjectModule() : ScriptModule("Object")
{
  ...
  bindFunction(0x00A0, game_get_object_position<CharacterObject>, 4,
  ...
  bindFunction(0x00F8, game_player_stopped_near_point_3d, 8,
               "Detect player stopped area 3d");
  ...
  bindFunction(0x0185, game_is_vehicle_health_greater_than, 2,
               "Is Vehicle Health Greater Than");
  ...
  bindFunction(0x01AD, game_locate_vehicle_near_point_2D, 6,
               "Locate Vehicle Near Point 2D");
  ...
  bindFunction(0x01C8, game_create_character_as_passenger, 5,
               "Create Character as Passenger");
  ...
  bindUnimplemented(0x020F, game_turn_character_to_face_player, 2,
  ...
  bindUnimplemented(0x0243, game_set_character_personality, 2,
  ...
  bindFunction(0x02DD, game_get_random_character_in_zone, 2,
               "Get Random Character In Zone");
  ...
  bindUnimplemented(0x03BC, game_create_cylinder, 5,
                    "Create Cylinder");  /// @todo Other sources call this "sphere", this
                                         /// should be our AreaIndicator?
