	...
	if (dynamics && body) {
		if (body->body->isStaticObject()) {
			if (_enablePhysics) {
				...
				btVector3 inert;
				body->body->getCollisionShape()->calculateLocalInertia(
				    dynamics->mass, inert);
		...
		if (inWater) {
			...
			if (wi != NO_WATER_INDEX) {
				...
				if (ws.z <= h) {
					...
					float F =
					    WATER_BUOYANCY_K * x +
					    -WATER_BUOYANCY_C * body->body->getLinearVelocity().z();
					btVector3 forcePos =
					    btVector3(0.f, 0.f, 2.f)
					        .rotate(body->body->getOrientation().getAxis(),
					                body->body->getOrientation().getAngle());
					body->body->applyForce(btVector3(0.f, 0.f, F), forcePos);
				}
			}
		}
	}
...
glm::vec3 InstanceObject::getPosition() const { return position; }

glm::quat InstanceObject::getRotation() const { return rotation; }
...
bool InstanceObject::takeDamage(const GameObject::DamageInfo& dmg)
{
	...
	if (dynamics) {
		...
		if (dmg.impulse >= dynamics->uprootForce &&
		    (body->body->getCollisionFlags() & btRigidBody::CF_STATIC_OBJECT) !=
		        0) {
			_enablePhysics = true;
		}
	}
