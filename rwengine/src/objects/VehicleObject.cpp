...
glm::quat VehicleObject::getRotation() const
{
	if (physVehicle) {
		btQuaternion rot =
		    physVehicle->getChassisWorldTransform().getRotation();
...
void VehicleObject::tickPhysics(float dt)
{
	...
	if (physVehicle) {
			...
			if (isInWater()) {
				...
				btVector3 rudderPoint =
				    btVector3(0.f, -info->handling.dimensions.y / 2.f, 0.f)
				        .rotate(orient.getAxis(), orient.getAngle());
				physBody->applyForce(rForce, rudderPoint);

				btVector3 rudderVector =
				    btVector3(0.f, 1.f, 0.f)
				        .rotate(orient.getAxis(), orient.getAngle());
				physBody->applyForce(rudderVector * engineForce * 100.f,
				                     rudderPoint);

				btVector3 dampforce(10000.f * velocity.x(),
				                    velocity.y() * 100.f, 0.f);
				physBody->applyCentralForce(
		...
		for (auto& it : dynamicParts) {
			...
			// auto p2 = inv * (glm::vec3(pos.x(), pos.y(), pos.z()) -
			// getPosition());
                                 ...
                                 VehicleObject::FrameState state)
{
	if (state == VehicleObject::OK) {
		if (part->normal)
			skeleton->setEnabled(part->normal, true);
		if (part->damaged)
			skeleton->setEnabled(part->damaged, false);
	} else if (state == VehicleObject::DAM) {
		if (part->normal)
			skeleton->setEnabled(part->normal, false);
		if (part->damaged)
			skeleton->setEnabled(part->damaged, true);
	}
}
	...
	if (wi != NO_WATER_INDEX) {
		...
		if (ws.z <= h) {
			...
			float F = WATER_BUOYANCY_K * x +
			          -WATER_BUOYANCY_C * physBody->getLinearVelocity().z();
...
void VehicleObject::createObjectHinge(btTransform& local, Part* part)
{
	...
	if (okframe->getGeometries().size() == 0)
		return;
	...
	auto hinge = new btHingeConstraint(*physBody, *subObject, tr.getOrigin(),
