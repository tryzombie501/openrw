...
/**
 * Finds the 'real' case for a path, to get around the fact that Rockstar's data
 * is usually the wrong case.
...
std::string findPathRealCase(const std::string& base, const std::string& path)
{
	...
	std::transform(orgFileName.begin(), orgFileName.end(), orgFileName.begin(),
	               ::tolower);
	std::string realName;
	...
	if (dp != NULL) {
		while ((ep = readdir(dp))) {
			...
			std::transform(lowerRealName.begin(), lowerRealName.end(),
			               lowerRealName.begin(), ::tolower);
			if (lowerRealName == orgFileName) {
				closedir(dp);
				if (isDirectory) {
					return findPathRealCase(base + "/" + realName,
					                        path.substr(endslash + 1));
				} else {
