...
void MenuState::enterMainMenu()
{
	...
	m->addEntry(Menu::lambda(
	    "Start", [=] { StateManager::get().enter(new IngameState(game)); }));
	m->addEntry(Menu::lambda("Load Game", [=] { enterLoadMenu(); }));
	m->addEntry(Menu::lambda("Test", [=] {
		StateManager::get().enter(new IngameState(game, true, "test"));
	}));
	m->addEntry(
	    Menu::lambda("Options", [] { RW_UNIMPLEMENTED("Options Menu"); }));
...
void MenuState::enter() { game->getWindow().setMouseCursorVisible(true); }
